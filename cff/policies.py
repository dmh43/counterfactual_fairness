...
class NaivePolicy:
  ...
  def train(self, R, S, G, L, F):
    self.F_reg = RidgeCV().fit(np.hstack([R, S, G, L]), F)
  ...
  def evaluate(self, R, S, G, L, nb_seats=None):
    ...
    if nb_seats is None:
      ...
      assert isinstance(nb_seats, int) and (nb_seats > 0)
      nb_seats = min(nb_obs, nb_seats)
    F_hat = self.F_reg.predict(np.hstack([R, S, G, L]))
...
class FairPolicy:
  ...
  def infer_A(self, R, S, G, L):
    G_err = G - self.G_reg.predict(np.hstack([R, S]))
    L_err = L - self.L_reg.predict(np.hstack([R, S]))
    return self.A_reg.transform(np.hstack([G_err, L_err]))
